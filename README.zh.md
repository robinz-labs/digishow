...
- **Cue 播放器**  
	...
	提供基于时间线的图形化界面，可为特定场景设计信号输出曲线，并可附加到预设中实现同步播放。
...
- **脚本支持**  
	...
	允许使用 JavaScript/QML 表达式和脚本，实现更丰富的互动逻辑。

- **网络协议**  

	支持 OSC 及基于 WebSocket 的管道用于外部扩展，同时支持 Art-Net 用于大型灯光控制系统。透过这些网络协议也能更方便地把 DigiShow 接入你的 AI 智能体。
...
7. **screen** 
	使用“屏幕”接口来动态展示图片/视频， 并对其缩放率、半透明度和位置偏移等进行动画处理。 

8. **osc** 
	DigiShow 中的信号变量可以透过 OSC 与外部软件进行实时交换，示例涵盖：TouchDesigner、Unity3D、Blender 以及 Python（可对接 AI 智能体）。还能利用 ESP32 的 WiFi OSC 功能，打造能够与 DigiShow 无线连接的互动小设备。
...
13. **arduino rioc** 
	测试 DigiShow 和你的 Arduino 或 Aladdin 硬件一起工作。
	
14. **messenger** 
	DigiShow 和你的 RFID 读卡器、G-Code 执行器等硬件之间收发特定串口消息。
	
15. **lfo**
	利用 LFO 产生的波形来推动信号输出，获得动画效果。还能利用 LFO 在工作中产生的时间码来触发随时间变化的动作。
